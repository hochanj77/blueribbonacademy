  ...
  return (
    <div>
      <PageHero
        ...
        subtitle="Fill out the form below and we'll send you our complete course catalog."
      />

      <section className="py-12 md:py-20 bg-background" ref={formSection.ref}>
        <div className="container mx-auto px-4">
          <div className={cn(
            "max-w-2xl mx-auto transition-all duration-700",
            formSection.inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
          )}>
            <div className="bg-card rounded-2xl p-6 md:p-10 shadow-xl border border-border/40">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="studentName">Student's Name (First, Last) *</Label>
                    <Input id="studentName" placeholder="Student's Name" value={formData.studentName} onChange={(e) => handleChange("studentName", e.target.value)} maxLength={200} required />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="phone">Phone Number</Label>
                    <Input id="phone" type="tel" placeholder="Phone Number" value={formData.phone} onChange={(e) => handleChange("phone", e.target.value)} maxLength={20} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input id="email" type="email" placeholder="Email" value={formData.email} onChange={(e) => handleChange("email", e.target.value)} maxLength={255} required />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="language">Preferred Language</Label>
                    <Select value={formData.preferredLanguage} onValueChange={(value) => handleChange("preferredLanguage", value)}>
                      <SelectTrigger><SelectValue placeholder="Eng / Kor" /></SelectTrigger>
                      <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="message" placeholder="Type your message here..." value={formData.message} onChange={(e) => handleChange("message", e.target.value)} maxLength={1000} rows={4} />
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Label htmlFor="terms" className="text-sm font-medium cursor-pointer">I accept terms & conditions</Label>
                  </div>
                </div>

                <div className="text-center pt-2">
                  <Button type="submit" variant="accent" size="xl" disabled={isSubmitting} className="rounded-full gap-2 group">
